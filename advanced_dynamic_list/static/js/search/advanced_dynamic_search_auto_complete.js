odoo.define('advanced_dynamic_list.auto_complete', function (require) {
    "use strict";
    ...
    return Widget.extend({
        template: "advanced_dynamic_list.auto_complete",
        ...
        start: function () {
            ...
            core.bus.on('click', this, function (ev) {
                // check if is not inside the input
                if (ev.target !== this.$input[0] && !$.contains(this.$el, ev.target)) {
                    self.close();
                }
            });
        },
        ...
        _updatePosition: function () {
            // get parent position
            var parent_offset = this.$input.offset();
            this.$el.css({
                top: parent_offset.top + this.$input.outerHeight(),
